      ...
      <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Hiragino Mincho ProN','Yu Mincho',Georgia,serif", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ background: C.paper, borderTop: `1px solid ${C.border}`, padding: "12px 18px", maxWidth: "640px", width: "100%", margin: "0 auto", boxSizing: "border-box", display: "flex", gap: "10px" }}>
          <textarea value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder="アドラーに話してみてください（Enterで送信）"
            rows={2} style={{ flex: 1, border: `1px solid ${C.border}`, padding: "11px 14px", fontSize: "13.5px", fontFamily: "inherit", resize: "none", lineHeight: "1.65", outline: "none", background: C.bg, color: C.ink, minHeight: "48px", maxHeight: "100px" }} />
          <button onClick={send} disabled={!input.trim() || loading}
            style={{ background: input.trim() && !loading ? C.bgDark : C.border, color: "#e8dfc8", border: "none", padding: "0 18px", fontSize: "12px", cursor: input.trim() && !loading ? "pointer" : "not-allowed", fontFamily: "inherit" }}>送信</button>
        </div>
